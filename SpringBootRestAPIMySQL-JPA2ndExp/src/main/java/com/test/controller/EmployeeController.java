...
public class EmployeeController {
	...
	@GetMapping("/bymanager/{manager}")
	@Operation(summary="get employee by manager")
	public ResponseEntity<List<Employee>> getEmployeeByManager(@PathVariable String manager){
	    List<Employee> list = service.getEmployeesByManager(manager);
	    if(list.size()>0) {
	        return new ResponseEntity<>(list, HttpStatus.OK);
	    } else {
	        return new ResponseEntity<>(list, HttpStatus.NOT_FOUND);
	...
	@Operation(summary="get employee by salary")
	public ResponseEntity<List<Employee>> getEmployeeBySalary(@PathVariable int salary){
	    List<Employee> list = service.getEmployeesBySalary(salary);
	    if(list.size()>0) {
	        return new ResponseEntity<>(list, HttpStatus.OK);
	    } else {
	        return new ResponseEntity<>(list, HttpStatus.NOT_FOUND);
